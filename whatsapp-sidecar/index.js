...
app.post('/sessions/:id/pairing-code', async (req, res, next) => {
  try {
    ...
    if (s.status === 'ready' || s.status === 'authenticated') {
      return res.status(409).json({ error: `session already paired (status: ${s.status})` });
    }
    const b = req.body || {};
    const phoneNumber = String(b.phoneNumber || '').replace(/\D+/g, '');
    if (!phoneNumber) {
      return res.status(400).json({ error: 'phoneNumber is required (international format, no +)' });
    }
    if (s.pairingCode) {
      return res.json({ id: req.params.id, status: s.status, code: s.pairingCode });
    }
    // requestPairingCode needs the browser page (this.pupPage) to be up.
    const client = s.client;
    const deadline = Date.now() + 20000;
    while (!client.pupPage && Date.now() < deadline) {
      await new Promise((resolve) => setTimeout(resolve, 250));
    }
    if (!client.pupPage) {
      return res.status(500).json({ error: 'client page not ready yet, try again in a moment' });
    }
    const code = await client.requestPairingCode(phoneNumber, true, b.intervalMs || 60000);
    s.pairingCode = code;
    s.status = 'qr';
